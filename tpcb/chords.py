...
def transposition_tone(tone, shift):
    ...
    dict_shifts = dict(
        [[semitones[i], semitones[(i + shift) % len(semitones)]]
...
def transposition_chord(chord, shift):
    ...
    if chord.find(" ") != -1:
        raise (NotImplementedError(
            "Unimplemented format of the chord '{}'. Chords cannot contain any spaces."
            .format(chord)))
    ...
    if chord[:2] in semitones:
        ...
        raise (NotImplementedError(
            "Unimplemented format of the chord '{}'".format(chord)))
...
def transposition_song(text, shift):
        ...
        for j in range(1, len(parts)):
            ...
            if parts[j].find(" ", right_index + 1) == right_index + 1:
                ...
                if end_match == None:
                    ...
                    end_index = re.search(
                        "\s*$", parts[j][:end_match.start()]).start()
                    star = False
            if end_index != -1:
    ...
    for i in range(1, len(parts)):
        ...
        assert original_chord.find(
            "\\"
        ) == -1, "Chord '{}' contains forbidden character '\\''".format(
            original_chord)
